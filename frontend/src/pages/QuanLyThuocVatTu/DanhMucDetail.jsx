...
import { 
  ...
  clearError,
  clearSuccess 
} from '../../features/thuocDonViTinh/thuocDonViTinhSlice';
...
export default function DanhMucDetail({ mode }) {
  ...
  const { current, loading, error, success } = useSelector(state => state.thuocDonViTinh);
  ...
  const onSubmit = (data) => {
    if (isNew) {
      dispatch(createDonViTinh(data));
    } else if (isEdit) {
              dispatch(updateDonViTinh({ id, data }));
    }
  };
      ...
      <div className="row">
        <div className="col-12">
          <h1 className="h3 mb-4">
            {isNew ? 'Thêm danh mục' : isEdit ? 'Chỉnh sửa danh mục' : 'Xem chi tiết danh mục'}
          </h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="row">
              ...
              <div className="col-md-6 mb-3">
                ...
                <input
                  {...register('donViTinh')}
                  className={`form-control ${errors.donViTinh ? 'is-invalid' : ''}`}
                  disabled={isView}
                />
                {errors.donViTinh && (
                  <div className="invalid-feedback">{errors.donViTinh.message}</div>
                )}
              </div>
            </div>
